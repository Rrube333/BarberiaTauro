...
export default function App() {
  ...
  if (screen === 'admin' && !session) {
    return (
      <View style={styles.container}>
        ...
        <TouchableOpacity style={styles.backBtn} onPress={() => setScreen('reserva')}>
  ...
  return (
    <View style={styles.container}>
      <View style={{ flex: 1 }}>
        {screen === 'reserva' ? <ReservaScreen /> : <AdminScreen session={session} />}
      </View>
...
const styles = StyleSheet.create({
  ...
  loadingBox: { flex: 1, backgroundColor: DARK, alignItems: 'center', justifyContent: 'center' },
  loadingTitle: { fontSize: 36, letterSpacing: 8, color: GOLD, fontWeight: '700' },
  tabBar: {
    flexDirection: 'row',
  ...
  tabLabel: { fontSize: 11, color: MUTED, letterSpacing: 1, textTransform: 'uppercase' },
