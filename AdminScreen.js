      ...
      <View style={modal.overlay}>
        <View style={modal.box}>
          <Text style={modal.title}>
            {barbero?.id ? "EDITAR BARBERO" : "NUEVO BARBERO"}
          </Text>
          ...
          <View style={{ flexDirection: "row", gap: 10, marginTop: 8 }}>
            ...
            <TouchableOpacity
              style={[
                modal.btnSave,
                (loading || uploadingFoto) && { opacity: 0.5 },
              ]}
              onPress={guardar}
        ...
        <View style={[modal.box, { maxHeight: "85%" }]}>
          ...
          <ScrollView style={{ maxHeight: 400 }}>
            ...
            {!loading && buscado && resultados.length === 0 && (
              <Text
                style={{ color: MUTED, textAlign: "center", marginTop: 20 }}
              >
              ...
              <View key={c.id} style={hist.row}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={hist.meta}>
                    {c.barberos?.nombre} · {formatFecha(c.fecha)} ·{" "}
                ...
                <View
                  style={[
                    hist.badge,
                    {
                      borderColor: estadoColor[c.estado] || MUTED,
                      backgroundColor: (estadoColor[c.estado] || MUTED) + "22",
                    },
                  ]}
                >
                  <Text
                    style={[
                      hist.badgeText,
                      { color: estadoColor[c.estado] || MUTED },
                    ]}
                  >
                    {c.estado}
                  </Text>
          ...
          <TouchableOpacity
            style={[modal.btnCancel, { marginTop: 12 }]}
            onPress={cerrar}
          >
...
function BloqueoModal({ visible, onClose, barberos }) {
  ...
  async function guardar() {
    if (!barberoId || !fecha) {
      Alert.alert("Selecciona barbero y fecha");
      return;
    }
    if (!bloqueoDia && slotsBlock.length === 0) {
      Alert.alert("Selecciona horarios o activa día completo");
      return;
    }
    setLoading(true);
        ...
        <View style={[modal.box, { maxHeight: "90%" }]}>
          ...
          >
            ...
            <TouchableOpacity
              style={[
                blq.chip,
                bloqueoDia && blq.chipActive,
                { marginBottom: 14, alignSelf: "flex-start" },
              ]}
              onPress={() => {
                setBloqueoDia(!bloqueoDia);
                setSlotsBlock([]);
              }}
            >
              <Text style={[blq.chipText, bloqueoDia && { color: GOLD }]}>
                ...
                >
                  {slots.map((s) => (
                    <TouchableOpacity
                      ...
                      style={[
                        blq.slotChip,
                        slotsBlock.includes(s) && blq.slotChipActive,
                      ]}
                      onPress={() =>
                        setSlotsBlock((prev) =>
                          prev.includes(s)
                    ...
                    >
                      <Text
                        style={[
                          ...
                          slotsBlock.includes(s) && { color: RED },
                        ]}
                      >
                        {s}
                      </Text>
            ...
            {bloqueos.length > 0 && (
              <>
                <Text style={[modal.label, { marginBottom: 10 }]}>
                  BLOQUEOS ACTIVOS
                </Text>
                {bloqueos.map((b) => (
                  <View key={b.id} style={blq.bloqueoRow}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text
                        style={{ color: MUTED, fontSize: 11, marginTop: 2 }}
                      >
                        {b.dia_completo
                          ? "🔒 Día completo"
                          : `🕐 ${(b.slots_bloqueados || []).join(", ")}`}
                      </Text>
                    </View>
                    <TouchableOpacity
          ...
          <TouchableOpacity
            style={[modal.btnCancel, { marginTop: 10 }]}
            onPress={onClose}
          >
            <Text style={modal.btnCancelText}>Cerrar</Text>
          </TouchableOpacity>
...
export default function AdminScreen({ session }) {
  ...
  async function toggleBarbero(id, activo) {
    ...
    setBarberos((prev) =>
      prev.map((b) => (b.id === id ? { ...b, activo: !activo } : b)),
    );
  }
  ...
  const citasPendientes = citas.filter(
    (c) => c.estado === "confirmada" && c.fecha >= hoy,
  ).length;
        ...
        {tab === "citas" && (
          <View>
            ...
            <View style={{ flexDirection: "row", gap: 10, marginBottom: 16 }}>
              ...
              >
                <Text style={[styles.quickBtnText, { color: GOLD }]}>
                  🔍 Historial cliente
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                <Text style={[styles.quickBtnText, { color: MUTED }]}>
                  🔒 Bloquear días
                </Text>
              </TouchableOpacity>
            </View>
            ...
            >
              <View style={{ flexDirection: "row", gap: 8 }}>
                {["todos", "confirmada", "completada", "cancelada"].map(
                  (est) => (
                    <TouchableOpacity
                      key={est}
                      style={[
                        styles.filterChip,
                        filtroEstado === est && styles.filterChipActive,
                      ]}
            ...
            >
              <View style={{ flexDirection: "row", gap: 8 }}>
                <TouchableOpacity
                  style={[
                    styles.filterChip,
                    filtroBarbero === "todos" && styles.filterChipActive,
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.filterText,
                      filtroBarbero === "todos" && { color: GOLD },
                    ]}
                  >
                    Todos
                  </Text>
                ...
                {barberos.map((b) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.filterChip,
                      filtroBarbero === b.id && styles.filterChipActive,
                    ]}
            ...
            {loading ? (
              ...
              <Text
                style={{ color: MUTED, textAlign: "center", marginTop: 40 }}
              >
                No hay citas
              </Text>
            ) : (
              citasFiltradas.map((c) => (
                <View
                  ...
                  style={[
                    styles.citaCard,
                    c.fecha === hoy && { borderColor: GOLD + "66" },
                  ]}
                >
                ...
                <View key={b.id} style={styles.barberoCard}>
                  <View style={styles.barberoInfo}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text
                        style={[
                          styles.barberoNombre,
                          !b.activo && { color: MUTED },
                        ]}
                      >
                        {b.nombre}
                      </Text>
                      <Text style={styles.barberoSpec}>
                        {b.especialidad || "Corte de cabello"}
                      </Text>
                      <Text style={styles.barberoSpec}>
                        {totalCitas} citas ·{" "}
                  ...
                  <View style={{ gap: 6 }}>
                    ...
                    <TouchableOpacity
                      style={[
                        styles.actionBtn,
                        { borderColor: b.activo ? RED : GREEN },
                      ]}
                    ...
                    >
                      <Text style={[styles.actionBtnText, { color: "#555" }]}>
                        🗑 Eliminar
                      </Text>
                    </TouchableOpacity>
                  </View>
